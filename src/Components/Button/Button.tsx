...
export const Button: FC<ButtonProps> = ({
  ...
  const baseStyle = `focus:outline-none transition-all duration-150 ease-in-out flex items-center rounded-[6px] ${padding} text-center bg-[#3D3D3D]`;
  const className = `
    ${baseStyle}
    ${variants[variant]}
    ...
    >
      ...
      {label && !loading && (
        <Typography
          type="l2"
          bold
          otherClasses="pointer-events-none p-0 m-0 !mb-[0px] overflow-hidden uppercase text-center w-full"
          gutterBottom={false}
        >
          {label}
        </Typography>
